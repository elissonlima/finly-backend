...
pub async fn update_category(
    req: HttpRequest,
    body: web::Json<UpdateCategoryReq>,
    app_state: web::Data<app_state::AppState>
) -> Result<HttpResponse, AppError> {
    let ext = req.extensions();
    let user_id = macros::get_user_id!(ext);
    ...
    let res = macros::unwrap_res_or_app_err_log_err!(
        category_controller.update(&body.category_id, &body.name, &body.icon_id, &body.color).await,
        AppError::InternalServerError,
        "An error occurred when tried to insert new category into database"
    );
...
pub async fn delete_category(
    ...
    match category_controller.delete(&body.category_id).await {
        Ok(_) => {}
        Err(e) => {
            log::error!("It wasn't possible to execute category deletion: {}", e);
